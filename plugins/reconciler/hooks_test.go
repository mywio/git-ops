...
func TestFetchRepoHooksPreStageContinuesWhenHookContentDecodeFails(t *testing.T) {
	server, client := newHookContentsDecodeFailureServer(t)
	defer server.Close()
	...
	err := reconciler.fetchRepoHookScriptsForStage(context.Background(), "acme", "api", "pre", localDir)

	require.NoError(t, err)
	content, readErr := os.ReadFile(filepath.Join(localDir, ".deploy", "pre", "ok.sh"))
	require.NoError(t, readErr)
	assert.Equal(t, "#!/bin/sh\necho ok\n", string(content))

	_, statErr := os.Stat(filepath.Join(localDir, ".deploy", "pre", "bad.sh"))
	assert.ErrorIs(t, statErr, os.ErrNotExist)
}
...
func newHookContentsDecodeFailureServer(t *testing.T) (*httptest.Server, *github.Client) {
	t.Helper()
	...
	mux.HandleFunc("/repos/acme/api/contents/.deploy/pre", func(w http.ResponseWriter, r *http.Request) {
		...
		require.NoError(t, json.NewEncoder(w).Encode([]map[string]any{
			{"type": "file", "name": "ok.sh", "path": ".deploy/pre/ok.sh"},
			{"type": "file", "name": "bad.sh", "path": ".deploy/pre/bad.sh"},
		}))
	})
	...
	mux.HandleFunc("/repos/acme/api/contents/.deploy/pre/bad.sh", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		require.NoError(t, json.NewEncoder(w).Encode(map[string]any{
			"type":     "file",
			"name":     "bad.sh",
			"path":     ".deploy/pre/bad.sh",
			"encoding": "base64",
			"content":  "!!!not-base64!!!",
		}))
	})
