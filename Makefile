...
build-plugins:
	...
	go build -buildmode=plugin -o $(PLUGINS_DIR)/audit.so plugins/audit/*.go
	go build -buildmode=plugin -o $(PLUGINS_DIR)/compose_refresh.so plugins/compose_refresh/*.go
